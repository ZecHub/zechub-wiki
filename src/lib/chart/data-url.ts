export const DATA_URL = {
  defaultUrl: "/data/zcash/shielded_supply.json",
  sproutUrl: "/data/zcash/sprout_supply.json",
  saplingUrl: "/data/zcash/sapling_supply.json",
  orchardUrl: "/data/zcash/orchard_supply.json",
  txsummaryUrl: "/data/zcash/transaction_summary.json",
  transparentSupplyUrl: "/data/zcash/transparent_supply.json",
  netInflowsOutflowsUrl: "/data/zcash/netinflowoutflow.json",
  nodecountUrl: "/data/zcash/nodecount.json",
  difficultyUrl: "/data/zcash/difficulty.json",
  lockboxUrl: "/data/zcash/lockbox.json",
  shieldedTxCountUrl: "/data/zcash/shieldedtxcount.json",
  issuanceUrl: "/data/zcash/issuance.json",
  zcashShieldedStatsUrl : "/data/zcash/shieldedStatsJSON",
  shieldedUrl:
    "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/shielded_supply.json",
  namadaSupplyUrl: "/data/namada/namada_supply.json",
  blockchainInfoUrl: "/api/blockchain-info",
  blockchairUrl:
    "https://api.blockchair.com/zcash/stats?key=A___eEXBXdQS8WpQXTDwrSRNOm8LJvfv",
  namadaRewardUrl: "/data/namada/namada_rewards_rate.json",
} as const;


export const DATE_URL = {
  defaultUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/shielded_supply.json&per_page=1",
  sproutUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/sprout_supply.json&per_page=1",
  saplingUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/sapling_supply.json&per_page=1",
  orchardUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/orchard_supply.json&per_page=1",
  txsummaryUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/transaction_summary.json&per_page=1",
  transparentSupplyUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/transparent_supply.json&per_page=1",
  netInflowsOutflowsUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/netinflowoutflow.json&per_page=1",
  nodecountUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/nodecount.json&per_page=1",
  difficultyUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/difficulty.json&per_page=1",
  lockboxUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/lockbox.json&per_page=1",
  shieldedTxCountUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/shieldedtxcount.json&per_page=1",
  issuanceUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/issuance.json&per_page=1",
  shieldedUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/shielded_supply.json&per_page=1",
  namadaSupplyUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/namada/namada_supply.json&per_page=1",
  blockchainInfoUrl: "/api/blockchain-info",
  blockchairUrl: "https://api.blockchair.com/zcash/stats?key=A___wNpGniYbmkv7tLpMfOPBGe2as7vI",
  namadaRewardUrl: "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/namada/namada_rewards_rate.json&per_page=1",
  zcashShieldedStatsUrl : "https://api.github.com/repos/ZecHub/zechub-wiki/commits?path=public/data/zcash/shieldedStatsJSON&per_page=1",
} as const;

