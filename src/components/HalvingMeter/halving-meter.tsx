...
import { MdOutlineDarkMode as DarkIcon, MdLightMode as LightIcon } from "react-icons/md";
...
// NavLinks: desktop
const NavLinks: React.FC<{ classes: string; closeMenu: () => void }> = ({ classes, closeMenu }) => (
  <div className={`flex jen-justify-content ${classes}`}>
      ...
      item.links ? (
        ...
        <div className="dropdown" key={item.name}>
          <Link
            ...
            onClick={closeMenu}
            className="jen-padding hover:text-yellow-300 dark:hover:text-yellow-500 hover:font-bold"
          >
            {item.name}
          </Link>
        </div>
    ...
    <div className="flex xl:flex-row flex-col xl:space-x-3 gap-2 ml-6">
      <Link
        href="/dao"
        onClick={closeMenu}
        className="p-2 border-4 border-light-blue-500 rounded-xl hover:bg-[#1984c7] hover:text-white dark:hover:bg-white dark:hover:text-black text-center w-16"
      >
        DAO
      </Link>
      <Link
        href="/dashboard"
        onClick={closeMenu}
        className="p-2 border-4 border-light-blue-500 rounded-xl hover:bg-[#1984c7] hover:text-white dark:hover:bg-white dark:hover:text-black text-center"
      >
        Dashboard
      </Link>
    </div>
...
// MobileNavLinks: mobile dropdown
const MobileNavLinks: React.FC<{ classes: string; closeMenu: () => void }> = ({ classes, closeMenu }) => (
      ...
      item.links ? (
        ...
        <div className="dropdown" key={item.name}>
          <Link
            ...
            target={item.newTab ? "_blank" : "_self"}
            onClick={closeMenu}
            className="jen-padding hover:text-yellow-300 dark:hover:text-yellow-500 hover:font-bold block p-2"
          >
            {item.name}
          </Link>
        </div>
...
// MobileNav: sheet wrapper
const MobileNav: React.FC<{ closeMenu: () => void }> = ({ closeMenu }) => (
  <div className="relative flex flex-col w-11/12 h-auto justify-center z-10">
    <div className="absolute top-20 left-0 right-0 p-6 bg-white dark:bg-slate-900 rounded-xl transition duration-200">
      <MobileNavLinks classes="flex-col font-bold gap-2 text-lg" closeMenu={closeMenu} />
      <div className="mt-6">
        <SocialIcons newTab />
      </div>
    </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
            ...
            <SheetContent side="left" className="p-0">
              <MobileNav closeMenu={() => setSheetOpen(false)} />
            </SheetContent>
          </Sheet>
        </div>
      </div>
