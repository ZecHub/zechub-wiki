...
const DATA_URL = "https://raw.githubusercontent.com/ZecHub/zechub-wiki/main/public/data/transaction_summary.json";

// Mapping block height to year boundaries
    ...
    async function fetchData() {
      try {
        ...
        rawData.forEach((item) => {
          const year = HEIGHT_YEAR_MAP.find(range => item.height >= range.start && item.height <= range.end)?.year ?? "Unknown";
    ...
    return Array.from(data.entries()).map(([year, value], index) => {
      ...
      return (
        <g key={year} transform={`translate(${xOffset}, 200)`}>
          <circle cx={0} cy={0} r={circleRadius} fill={color} fillOpacity={0.2} stroke={color} strokeWidth={2} />
          <text x={0} y={-circleRadius + 20} textAnchor="middle" fill={color} fontSize="14" fontWeight="bold">
            {year}
          </text>
          <text x={0} y={-circleRadius + 40} textAnchor="middle" fill={color} fontSize="12">
  ...
  return (
    <div style={{ textAlign: "center", backgroundColor: "#f8f4e8", padding: "20px" }}>
      <svg width={1000} height={500}>
        ...
        <text x={800} y={250} textAnchor="middle" fill="#333" fontSize="16" fontWeight="bold">
          Sapling & Orchard
        </text>
