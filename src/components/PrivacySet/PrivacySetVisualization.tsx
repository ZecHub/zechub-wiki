...
const PrivacySetVisualization: React.FC = () => {
  ...
  // Raw counts
  const rawSap: [string, number][] = saplingYears.map(
    (y) => [y, yearly.get(y)?.sapling || 0]
  );
  const rawOrch: [string, number][] = orchardYears.map(
    (y) => [y, yearly.get(y)?.orchard || 0]
  );
    ...
    data.map(([year,val], idx) => {
      ...
      return (
        ...
        >
          ...
          />
          <text x={0} y={-r-10} textAnchor="middle" fill="#333" fontSize={14} fontWeight="bold">
            {year}
          </text>
  ...
  return (
    <div style={{background:'#f8f4e8',padding:20,borderRadius:8}}>
      <svg width={1200} height={650} style={{margin:'0 auto',display:'block'}}>
        ...
        <circle cx={orcX} cy={350} r={maxR+10} fill="none" stroke="#111"   strokeOpacity={0.4} strokeWidth={4} />
