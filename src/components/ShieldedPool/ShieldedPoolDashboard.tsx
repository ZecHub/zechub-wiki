...
// VALID pool keys for TypeScript
...
export default function ShieldedPoolDashboard() {
  ...
  useEffect(() => {
    ...
    // TX & node counts
    fetchJSON<ShieldedTxCount[]>(DataUrlOptions.shieldedTxCountUrl).then(d => d && setShieldedTxCount(d));
    fetchJSON<NodeCountData[]>(DataUrlOptions.nodecountUrl).then(nodes => {
      if (nodes?.length) setLatestNodeCount(Number(nodes[nodes.length-1].nodecount));
    });

    // Namada raw + asset list
    fetchJSON<any[]>(DataUrlOptions.namadaSupplyUrl).then(data => {
      if (!data) return;
      setNamadaRaw(data);
  ...
  // Last-updated effect now uses getDataUrl(), never indexes with string
  useEffect(() => {
    (async () => {
      const url = getDataUrl();
      const arr = (await fetchJSON<SupplyData[]>(url)) || [];
  ...
  // Map selectedPool → URL
  function getDataUrl() {
    switch (selectedPool) {
      case "sproutUrl": return DataUrlOptions.sproutUrl;
      case "saplingUrl": return DataUrlOptions.saplingUrl;
      case "orchardUrl": return DataUrlOptions.orchardUrl;
  ...
  // Build Namada time series on asset change
  useEffect(() => {
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-6">
            {poolKeys.map(key=>(
              <div key={key} className="flex flex-col items-center">
                <Button
                  text={poolLabels[key]}
                  className={`px-4 py-2 rounded-full ${selectedPool===key?"bg-[#1984c7] text-white":"bg-gray-400 text-white"}`}
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-6">
            {namadaAssets.map(asset=>(
              <div key={asset.id} className="flex flex-col items-center">
                <Button
                  text={asset.id}
                  className={`px-4 py-2 rounded-full ${selectedNamadaAsset===asset.id?"bg-yellow-500 text-white":"bg-gray-400 text-white"}`}
                  onClick={()=>setSelectedNamadaAsset(asset.id)}
                />
                <span className="text-sm text-gray-600 mt-1">
                  {asset.id==="Namada"? Number(asset.totalSupply||0).toLocaleString(undefined,{maximumFractionDigits:2})
        ...
        <div className="flex justify-end items-center gap-4 mt-4">
          ...
          <Button
            ...
            onClick={()=>handleSaveToPng(selectedPool, {
              sproutSupply: supplies.sproutUrl,
              saplingSupply: supplies.saplingUrl,
              orchardSupply: supplies.orchardUrl,
            }, selectedTool)}
          />
        </div>
      </div>
