...
// Dynamically imported Zcash charts
...
export default function ShieldedPoolDashboard() {
  ...
  const [supplies, setSupplies]                       = useState<Record<"defaultUrl"|"sproutUrl"|"saplingUrl"|"orchardUrl", SupplyData|null>>({
    defaultUrl: null,
    sproutUrl: null,
    saplingUrl: null,
    orchardUrl: null,
  ...
  // === getDataUrl wrapped in useCallback ===
  ...
  useEffect(() => {
    ...
    fetchJSON<NodeCountData[]>(DataUrlOptions.nodecountUrl)
      .then(nodes => nodes?.length && setLatestNodeCount(Number(nodes[nodes.length-1].nodecount)));
  ...
  // === Namada series effect (always in same order) ===
  useEffect(() => {
