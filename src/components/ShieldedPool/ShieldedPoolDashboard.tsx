...
export default function ShieldedPoolDashboard() {
  ...
  const [supplies, setSupplies]                       = useState<Record<PoolKey,SupplyData|null>>({
    defaultUrl:null, sproutUrl:null, saplingUrl:null, orchardUrl:null
  });
  ...
  useEffect(() => {
    ...
    (["defaultUrl","sproutUrl","saplingUrl","orchardUrl"] as PoolKey[]).forEach(key => {
      fetchJSON<SupplyData[]>(DataUrlOptions[key])
        .then(a => a && setSupplies(s => ({ ...s, [key]: a.pop()||null })));
    });

    // shielded counts + nodes
    ...
    fetchJSON<any[]>(DataUrlOptions.namadaSupplyUrl)
      .then(d => {
        if (!d) return;
        setNamadaRaw(d);
        const list = d[0]?.Total_Supply || [];
        setNamadaAssets(list);
        if (list.length) setSelectedNamadaAsset(list[0].id);
      });
  }, []);

  // update last-updated whenever URL changes
  useEffect(() => {
    (async () => {
  ...
  // ensure pool only changes for supply charts
  ...
  return (
    <div className="mt-28">
      ...
      <div className="border p-4 rounded-lg relative">
        {/* only supply for Namada */}
        {selectedCoin==="Namada" ? (
          <Tools
            onToolChange={t=>t===ToolOptions.supply && handleToolChange(t)}
            defaultSelected={ToolOptions.supply}
            options={[ToolOptions.supply]}
          />
        ) : (
          ...
          {selectedCoin==="Zcash" && <>
            ...
            {selectedTool===ToolOptions.transaction && <>
              ...
              <TransactionSummaryChart
                ...
                pool={selectedTool===ToolOptions.supply ? selectedPool : "defaultUrl"}
