...
const ShieldedPoolDashboard: React.FC = () => {
  // NOTE: select any string key, not just four literals
  ...
  // For Namada
  const [namadaAssets, setNamadaAssets] = useState<
    { id: string; totalSupply: string }[]
  >([]);
    ...
    (async () => {
      const key =
        selectedPool === "default"
          ? "defaultUrl"
          : (selectedPool + "Url") as keyof typeof DataUrlOptions;
  ...
  // fetch Namada assets when that coin selected
  useEffect(() => {
  ...
  const handleToolChange = (tool: ToolOptions) => {
    ...
    // reset pool for supply/transaction/privacy_set
    if (
