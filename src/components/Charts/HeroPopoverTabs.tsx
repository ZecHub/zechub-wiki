...
export default function HeroPopoverTabs({
  ...
  return (
    <div className="relative bg-card border border-border rounded-3xl p-4 sm:p-6 shadow-sm">
      <div className="flex items-center justify-center gap-4 sm:gap-6">
        {/* All Tabs trigger */}
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="flex items-center gap-3 px-5 py-3 bg-muted hover:bg-accent border border-border rounded-3xl text-muted-foreground hover:text-foreground transition-all duration-200 font-medium text-sm sm:text-base"
        >
        ...
        <div className="w-[120px] sm:w-[148px]" />
      </div>

      {/* Supply buttons - wrap on mobile */}
      {activeTab === "supply" && (
        <div className="flex flex-wrap justify-center gap-2 mt-5 sm:mt-6">
          {supplyOptions.map((option) => (
            <button
              ...
              className={`px-5 py-2.5 sm:px-7 sm:py-3 text-sm font-medium rounded-3xl transition-all whitespace-nowrap ${
      ...
      {/* All Tabs sub-row - responsive grid */}
      {isOpen && (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
            {otherTabs.map((tab) => (
              <button
                ...
                className={`px-4 py-3 text-sm font-medium rounded-3xl transition-all text-center ${
                  activeTab === tab.value
