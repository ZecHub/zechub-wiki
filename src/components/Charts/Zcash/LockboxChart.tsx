...
export default function LockboxChart(props: LockboxChartProps) {
  ...
  const handleCopyAddress = async () => {
    try {
      await navigator.clipboard.writeText("t3ev37Q2uL1sfTsiJQJiWJoFzQpDhmnUwYo");
      setCopied(true);
    ...
    <ErrorBoundary fallback={"Failed to load Lockbox Chart"}>
      ...
      <div className="grid grid-cols-3 gap-4 mt-8">
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-2xl p-4">
          <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">Lockbox Total</p>
          <p className="text-2xl font-semibold text-blue-700 dark:text-blue-300 mt-1">{lockboxTotal.toLocaleString()}</p>
        </div>
        <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-2xl p-4">
          <p className="text-xs text-red-600 dark:text-red-400 font-medium">Coinholders Fund Total</p>
          <p className="text-2xl font-semibold text-red-700 dark:text-red-300 mt-1">{coinholdersTotal.toLocaleString()}</p>
        </div>
