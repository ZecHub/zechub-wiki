      ...
      <SectionCard title={qs?.installation ?? "Installation"}>
        ...
        {osTab === "linux" && (
          <>
            ...
            <CodeBlock
              code={`cargo install --git https://github.com/ZcashFoundation/zebra --tag v6.2.3 zebrad`}
            />
          </>
        )}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "40px" }}>
      <section id="installing-zakura">
        ...
        <InfoBox>
          ...
          {qs?.zakuraInfo ??
            "is a consensus-compatible Zcash full node written in Rust and built for high-performance syncing, pruning, snapshots, and zcashd-compatible RPC workflows."}
        </InfoBox>
...
source "$HOME/.cargo/env"`}
            ...
            <SubLabel className="mt-[14px]">
              {qs?.fromCratesIo ?? "From crates.io"}
            </SubLabel>
            <CodeBlock code={`cargo install --locked zakura`} />
          </SectionCard>
          ...
          <SectionCard title={qs?.fromSource ?? "From GitHub"}>
            ...
            <CodeBlock code={`zakurad --version`} />
          </SectionCard>
  ...
  zakuracore/zakura:latest`}
            />
            <div className="mt-[14px] p-[12px_14px] rounded-lg text-[13px] leading-relaxed bg-amber-500/[0.06] border border-amber-500/20 text-amber-700 dark:text-amber-400">
              {qs?.zakuraDockerNote ??
                "Port 8233 exposes the Mainnet P2P service. Use port 18233 when configuring a Testnet container."}
            </div>
          </SectionCard>
        </div>
      </section>
      ...
      <section id="running-zakura">
        <div className="flex items-center gap-[14px] mb-7">
          <StepBadge number={2} />
          <h2 className="text-slate-900 dark:text-slate-100 text-[22px] font-bold m-0">
            {qs?.runningZakura ?? "Running Zakura"}
          </h2>
        </div>
        ...
        >
          <SectionCard title={qs?.commandLine ?? "Command Line"}>
            <SubLabel>{qs?.startNode ?? "Start the Node"}</SubLabel>
            <CodeBlock code={`zakurad start`} />

            <SubLabel className="mt-[14px]">
              {qs?.viewOptions ?? "View Available Options"}
            </SubLabel>
            <CodeBlock code={`zakurad start --help`} />
          </SectionCard>

          <SectionCard title={qs?.dockerManagement ?? "Docker Management"}>
            <SubLabel>{qs?.viewLogs ?? "View Logs"}</SubLabel>
            <CodeBlock code={`docker logs -f zakura`} />

            <SubLabel className="mt-[14px]">
              {qs?.stopAndRestart ?? "Stop and Restart"}
            </SubLabel>
            <CodeBlock
              code={`docker stop zakura
docker start zakura`}
            />
          </SectionCard>
        </div>
      ...
      <section className="rounded-[14px] p-8 mt-2 bg-sky-500/[0.03] dark:bg-sky-500/[0.05] border border-sky-500/15">
        <h3 className="text-slate-900 dark:text-slate-100 text-[18px] font-bold mb-6">
          {qs?.zakuraResources ?? "Zakura Resources"}
        </h3>
        <div
        ...
        >
          ...
          <ResourceLink href="https://github.com/zakura-core/zakura">
            {qs?.sourceRepository ?? "Source Repository"}
          </ResourceLink>
          <ResourceLink href="https://github.com/zakura-core/zakura/releases">
            {qs?.releases ?? "Releases"}
          </ResourceLink>
          <ResourceLink href="https://zakura.com/snapshots">
