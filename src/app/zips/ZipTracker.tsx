...
export default function ZipTracker({
  zips,
  lastSyncedAt,
  source,
  embedded = false,
  ...
  const filtered = useMemo(() => {
    ...
    let arr = zips.filter((z) => {
      ...
      const hay =
        `${z.num} zip-${z.num} ${z.title} ${z.tags.join(" ")} ${z.summary || ""}`.toLowerCase();
  ...
  const nu7Zips = useMemo(
    () => zips.filter((z) => z.tags.includes("nu7")),
    [zips],
  );

  const drawerZip =
    drawerNum != null ? zips.find((z) => z.num === drawerNum) : null;
  const sandboxZip =
    sandboxNum != null ? zips.find((z) => z.num === sandboxNum) : null;
  ...
  return (
    ...
    >
      ...
      {!embedded && (
        <section className="relative max-w-[1320px] mx-auto px-4 md:px-8 pt-12 md:pt-16 pb-8 md:pb-10 border-b border-zinc-200 dark:border-zinc-800">
          <div
            ...
            style={{
              background: "radial-gradient(circle, #F4B728, transparent 65%)",
            }}
            aria-hidden
          />
          ...
          >
            ...
            <br />
            <em className="not-italic italic text-[#1984c7] dark:text-[#3fa3e0]">
              in one place.
            </em>
          </h1>
          <p className="text-[16px] md:text-[17px] text-zinc-600 dark:text-zinc-300 max-w-[680px] leading-relaxed mb-7">
            Track every ZIP from Draft to Final. Filter by category, search by
            keyword, and follow what's queued for the next network upgrade — all
            kept in sync with the canonical{" "}
            <a
              href="https://github.com/zcash/zips"
              className="text-[#1984c7] dark:text-[#3fa3e0] hover:underline"
            >
              zcash/zips
            </a>{" "}
            repository.
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-y-5 md:gap-y-0 mt-8 pt-7 border-t border-zinc-200 dark:border-zinc-800">
            {(
              ["Final", "Active", "Proposed", "Draft", "Reserved"] as const
            ).map((k, i) => (
              <div
                key={k}
      ...
      <section
        ...
        style={{
          background: "linear-gradient(135deg, #0f1827 0%, #142943 100%)",
        }}
      >
        <div
        ...
        <div className="relative max-w-[1320px] mx-auto">
          <div className="flex justify-between items-baseline mb-5 flex-wrap gap-4">
            <div>
              <div
                className="text-[24px] md:text-[28px] tracking-tight font-medium"
                style={{ fontFamily: "'Fraunces', serif" }}
              >
              ...
              <div className="text-[13px] text-zinc-400 mt-2 max-w-[540px] leading-relaxed">
                These ZIPs are under consideration for deployment in NU7. Final
                inclusion will be defined by draft-arya-deploy-nu7. No decision
                has been made yet.
              </div>
            </div>
          </div>
          ...
          >
            {nu7Zips.map((z) => (
              ...
              >
                ...
                <div className="text-[14px] font-medium text-zinc-100 leading-snug">
                  {z.title}
                </div>
                {hasSandbox(z.num) && (
      ...
      {/* CONTROLS */}
      <section
        className={`max-w-[1320px] mx-auto pt-7 pb-3 flex flex-col gap-4 ${embedded ? "px-0 mt-6" : "px-4 md:px-8"}`}
      >
        ...
        <Filters
          ...
          activeColor={(id) =>
            id === "Final"
              ...
              : id === "Active"
                ? "bg-[#1984c7] border-[#1984c7] text-white"
                : id === "Proposed"
        ...
        <div className="flex justify-between items-center pt-1 text-[12px] text-zinc-500 dark:text-zinc-400">
          ...
          <div className="flex gap-3 items-center">
            ...
            <select
              ...
              className="bg-transparent border-0 text-[12px] text-zinc-900 dark:text-zinc-100 cursor-pointer px-2 py-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800 outline-none min-w-38"
            >
      ...
      {/* LIST */}
      <section
        className={`max-w-[1320px] mx-auto pt-2 ${embedded ? "px-0 pb-8" : "px-4 md:px-8 pb-20"}`}
      >
  ...
  return (
    <section className="mb-6">
      ...
      <div className="grid grid-cols-3 md:grid-cols-5 gap-y-3 md:gap-y-0 rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900/40 px-1 md:px-2 py-3">
        {(["Final", "Active", "Proposed", "Draft", "Reserved"] as const).map(
          (k, i) => (
            <div
              key={k}
...
function SyncIndicator({
  ...
  return (
    <span
      title={title}
      aria-label={isLive ? "live" : "fallback"}
  ...
  return (
    ...
    >
      <div className="font-mono text-[13px] font-semibold text-zinc-900 dark:text-zinc-100 tracking-wide">
        <span className="text-zinc-400 dark:text-zinc-500 font-medium md:inline">
          ZIP{" "}
        </span>
        {fmtZipNum(zip.num)}
      </div>
      ...
      <div className="flex gap-1.5 flex-wrap">
        {zip.tags.map((t) => (
          <span
            key={t}
            className={`font-mono text-[10px] px-2 py-0.5 rounded font-medium tracking-wide ${tagClass(t)}`}
          >
            {t}
          </span>
        ))}
      ...
      <div className="inline-flex items-center gap-2 text-[12px] font-medium text-zinc-700 dark:text-zinc-300">
        <span
          className={`w-2 h-2 rounded-full shrink-0 ${STATUS_DOT[zip.status]}`}
        />
        {zip.status}
      </div>
    ...
    <>
      ...
      >
        <div className="px-5 md:px-7 pt-5 md:pt-6 pb-5 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 bg-white dark:bg-[#0f1720] z-[2]">
          ...
          <div
            className="text-[20px] md:text-[26px] font-medium leading-tight tracking-tight mb-4 pr-10 break-words"
            style={{ fontFamily: "'Fraunces', serif" }}
          >
            {zip.title}
          </div>
          <div className="flex gap-2 flex-wrap items-center">
            <span className="inline-flex items-center gap-2 text-[12px] font-medium">
              <span
                className={`w-2 h-2 rounded-full ${STATUS_DOT[zip.status]}`}
              />
              {zip.status}
            </span>
            {zip.tags.map((t) => (
              ...
              {isTerminal ? (
                ...
                LIFECYCLE.map((s, i) => {
                  ...
                  return (
                    <div
                      key={s}
                      className="relative pb-5 last:pb-0 text-[13px]"
                    >
                      <span
                      ...
                      />
                      <div
                        className={`font-medium ${current ? "text-[#1984c7] dark:text-[#3fa3e0]" : "text-zinc-900 dark:text-zinc-100"}`}
                      >
                        {s}
                      </div>
                      <div className="text-zinc-500 dark:text-zinc-400 text-[12px] mt-0.5">
        ...
        <div className="px-5 md:px-7 py-5 flex flex-col md:flex-row gap-2.5 md:flex-wrap border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40">
          {hasSandbox(zip.num) && (
            <button
              ...
              style={{
                background: "linear-gradient(135deg, #F4B728, #e8a418)",
                color: "#0e1116",
              }}
            >
              ⚗ Open Sandbox
    ...
    <>
      ...
      >
          ...
          <div className="flex items-center gap-3.5 min-w-0">
            ...
            <div className="min-w-0">
              <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-zinc-500 dark:text-zinc-400 font-semibold mb-0.5">
                {zip
                  ? `ZIP ${fmtZipNum(zip.num)} · NU7 candidate · ${zip.status}`
                  : ""}
              </div>
              <div
        ...
        <div className="flex-1 overflow-y-auto bg-zinc-50 dark:bg-[#111b27]">
          ...
          {open && !entry && zip && (
            <PlaceholderSandbox num={zip.num} title={zip.title} />
          )}
        </div>
      </div>
    </>
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-4 md:px-9 pt-7 pb-16">
      <div className="rounded-md border border-dashed border-zinc-300 dark:border-zinc-700 px-8 py-16 text-center max-w-[540px] mx-auto mt-12 bg-white dark:bg-[#0f1720]">
        ...
        <h2
          className="text-[22px] font-medium text-zinc-900 dark:text-zinc-100 mb-2"
          style={{ fontFamily: "'Fraunces', serif" }}
        >
          Sandbox in development
        </h2>
        <p className="text-zinc-600 dark:text-zinc-300 text-[13.5px] leading-relaxed mb-1.5">
          The interactive sandbox for ZIP {fmtZipNum(num)} ({title}) is being
          built.
        </p>
      </div>
    </div>
