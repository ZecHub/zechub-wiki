...
let warnings = 0; // advisory: a brand carrying a non-verbatim value (editorial call)
const detail = [];
for (const loc of CANONICAL_LOCALES) {
  ...
  const missNav = [], missCoin = [], badBrand = [];
  ...
  if (missNav.length || missCoin.length || badBrand.length) {
    problems += missNav.length + missCoin.length;
    warnings += badBrand.length;
    detail.push({ loc, missNav, missCoin, badBrand });
  }
}
...
if (problems === 0) {
  ...
  // Print advisory brand warnings but do not fail on them.
  for (const d of detail) for (const b of d.badBrand ?? []) console.warn(`  warning ${d.loc}: brand should be verbatim: ${b}`);
  process.exit(0);
}
